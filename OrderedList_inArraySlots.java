...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots
        ...
        for (int i = 0; i < unordered.size(); i ++) {
            ...
            int smallestInt = unordered.get(nextLargerAt);
            int oldInt = unordered.get(i);
            unordered.set(nextLargerAt, oldInt);
            unordered.set(i, smallestInt);
    ...
    /** 
      ...
      @return the smallest value's index from i to n
     */
     private int champIndex(int start) {
        int minimum = Integer.MAX_VALUE;
         int index = start;
         for (int i = start; i < list_iAS.size(); i ++) {
             if (list_iAS.get(i) < minimum) {
                 minimum = list_iAS.get(i);
                 index = i;
             }
         }
        return index;
